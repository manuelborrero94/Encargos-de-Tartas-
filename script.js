...
document.getElementById("cakeOrderForm").addEventListener("submit", function(event) {
    ...
    const order = {
        ...
        cakePhoto: "" // Este campo se llenará después
    };

    // Verificar si se ha cargado una imagen
    const photoInput = document.getElementById("cakePhoto");
    if (photoInput.files && photoInput.files[0]) {
        const reader = new FileReader();
        reader.onload = function(e) {
            order.cakePhoto = e.target.result; // Guardar la imagen en base64
            ...
            displayOrders(); // Actualizar el historial de pedidos

            // Resetea el formulario
            document.getElementById("cakeOrderForm").reset();
            calculateTotal(); // Vuelve a calcular el total
        };
        reader.readAsDataURL(photoInput.files[0]); // Leer la imagen como una URL de datos
    } else {
        // Guardar el pedido en el almacenamiento local si no hay imagen
        let orders = JSON.parse(localStorage.getItem("orders")) || [];
        orders.push(order);
        localStorage.setItem("orders", JSON.stringify(orders));
...
function displayOrders() {
    ...
    orders.forEach(order => {
        ...
        orderDiv.innerHTML = `
            ...
            ${order.cakePhoto ? `<p><strong>Foto de la Tarta:</strong></p><img src="${order.cakePhoto}" alt="Foto de la Tarta" style="max-width: 200px; max-height: 200px;">` : ""}
